...
using namespace ghg::QJsonPrivate;
...
bool Parser::parseNumber(QJsonPrivate::Value *val)
{
    ...
    if (isInt) {
        ...
        bool ok;
        number.toDouble(&ok);
        const auto numberString = number.toStdString();
        QDecimal n(numberString);
        if (!ok || n.IsNan()) {
            lastError = QJsonParseError::IllegalNumber;
            return false;
        }
    ...
    *(QJsonPrivate::qle_ushort *)(data + stringPos) = ushort(current - stringPos - sizeof(ushort));
    pos = reserveSpace((4 - current) & 3);
    if (pos < 0)
        return false;
    while (pos & 3)
        data[pos++] = 0;
