    ...
    : d(0)
{
    ...
    case Bool:
        ...
        break;
    case Double: {
        QString s = v.asLatin1String(base).toString();
...
QJsonValue::QJsonValue(double n)
    : d(0), t(Double)
{
    stringDataFromQStringHelper(QString::number(n));
}

QJsonValue::QJsonValue(const QDecimal& decimal)
    : d(0), t(Double)
{
    auto stringRepresentation = decimal.ToString();
    stringDataFromQStringHelper(stringRepresentation.c_str());
}
...
{
    if ((t == String || t == Double) && stringData && !stringData->ref.deref())
        free(stringData);
...
QJsonValue::QJsonValue(const QJsonValue &other)
{
    ...
    if ((t == String || t == Double) && stringData)
        stringData->ref.ref();
}
...
double QJsonValue::toDouble(double defaultValue) const
{
    ...
    stringData->ref.ref();
    QStringDataPtr holder = { stringData };
    return QString(holder).toDouble();
}

QDecimal QJsonValue::toDecimal(const QString& defaultValue) const
{
    if (t != Double)
        return QDecimal(defaultValue.toStdString());

    stringData->ref.ref();
    QStringDataPtr holder = { stringData };
    const auto &stringRepresentation = QString(holder).toStdString();
    return QDecimal(stringRepresentation);
}
...
QDebug operator<<(QDebug dbg, const QJsonValue &o)
{
    ...
    case QJsonValue::Double: {
        auto decimalString = o.toDecimal().ToString();
        dbg.nospace() << "QJsonValue(double, " << decimalString.c_str() << ')';
        break;
    }
